...
%\VignetteIndexEntry{ChemmineDrugs}
-->
...
Searching is done with the *findCompounds* function from the *ChemmineR* package. 
...
```{r }
results = findCompounds(drugbankConn,"mw",c("mw < 200"))
...
```{r }
results = findCompounds(drugbankConn,c("mw","aromatic"),c("mw < 200","aromatic >= 2"))
message("found ",length(results))
```
...
```{r }
results = findCompounds(drugbankConn,"formula",c("formula like '%C6%'"))
message("found ",length(results))
```
...
```{r }
allIds = getAllCompoundIds(drugbankConn)
...
names = getCompoundNames(drugbankConn,results)

#if the name order is important set keepOrder=TRUE 
#It will take a little longer though
names = getCompoundNames(drugbankConn,results,keepOrder=TRUE) 


# get the whole set of compounds
sdfset = getCompounds(drugbankConn,results)
#in order:
sdfset = getCompounds(drugbankConn,results,keepOrder=TRUE)
#write results directly to a file:
sdfset = getCompounds(drugbankConn,results,filename=file.path(tempdir(),"results.sdf"))
```

Using the *getCompoundFeatures* function, you can get a set of feature values 
as a data frame:
```{r}
getCompoundFeatures(drugbankConn,results[1:5],c("mw","logp","formula"))

#write results directly to a CSV file (reduces memory usage):
getCompoundFeatures(drugbankConn,results[1:5],c("mw","logp","formula"),filename="features.csv")

#maintain input order in output:
print(results[1:5])
getCompoundFeatures(drugbankConn,results[1:5],c("mw","logp","formula"),keepOrder=TRUE)
```
