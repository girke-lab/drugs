...
```{r }
allIds = getAllCompoundIds(dudConn)
...
as a data frame:
```{r}
getCompoundFeatures(dudConn,results[1:5],c("mw","logp","cansmi"))

#write results directly to a CSV file (reduces memory usage):
getCompoundFeatures(dudConn,results[1:5],c("mw","logp","cansmi"),filename="features.csv")

#maintain input order in output:
print(results[1:5])
getCompoundFeatures(dudConn,results[1:5],c("mw","logp","cansmi"),keepOrder=TRUE)
```
